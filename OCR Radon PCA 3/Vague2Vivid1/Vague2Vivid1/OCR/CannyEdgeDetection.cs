...
namespace Vague2Vivid1.OCR
{
    ...
    class Canny 
    {
        ...
        private static Point[] edgePoints, allPoints;
        //public PointF[] edgePoints;
        private Point edgePoint;
        public static int count = 0 ;
        private int countAllPoints=0;

       // public int rit; /// testing count
        ...
        public Canny(Bitmap Input)
        {
            ...
            edgePoints = new Point[50000000];
            allPoints = new Point[50000000];
            edgePoint = new Point();
        ...
        private void ReadImage()
        {
            ...
            unsafe
            {
                ...
                for (i = 0; i < bitmapData1.Height; i++)
                {
                    for (j = 0; j < bitmapData1.Width; j++)
                    {
                        ...
                        if (GreyImage[j, i] == 255)
                        {
                            allPoints[countAllPoints].X = j;
                            allPoints[countAllPoints].Y = i;
                            countAllPoints++;
                        }
                        //4 bytes per pixel
                        imagePointer1 += 4;
            ...
            Console.WriteLine("count all points "+countAllPoints);
            image.UnlockBits(bitmapData1);
            return;
        }
        public Point[] getGreyImage()
        {
            return allPoints;
        }
        ...
        private void DetectCannyEdges()
        {
           ...
           // NonMax = Gradient;
            ...
            for (r = Limit; r <= (Width - Limit) - 1; r++)
            {
                for (c = Limit; c <= (Height - Limit) - 1; c++)
                {
                    if (PostHysteresis[r, c] >= MaxHysteresisThresh)
                    {
                        ...
                        //Console.Write("x = " + r);//added to find the edge points
                        //Console.WriteLine("\ty = " + c);//

                       // GeneralMatrix.Create(M);
                   ...
                   //     pca.covariance();
                        ...
                        edgePoint.Y=c;
                //Console.WriteLine("("+count+")X edge: " + edgePoint.X + " Y edge: " + edgePoint.Y);
                        edgePoints.SetValue(edgePoint, count);
                        count++;
                    ...
                    if ((PostHysteresis[r, c] < MaxHysteresisThresh) && (PostHysteresis[r, c] >= MinHysteresisThresh))
                    {
                        ...
                        edgePoint.X = r;
                        edgePoint.Y = c;
              // Console.WriteLine("X edge: " + edgePoint.X + " Y edge: " + edgePoint.Y);
                        edgePoints.SetValue(edgePoint, count);
                        count++;
            ...
            ////{
            ////    Console.WriteLine("edgepoints : " + edgePoints[p]);
            ////    //count--;
            ////}

            
            Console.WriteLine("The total edge pixels count: " + count);
        ...
        {
            //Console.WriteLine("edgepoints {}", edgePoints);
            return edgePoints;
        }
